...
void tst_QQuickListView::reuse_reuseIsOffByDefault()
{
    ...
    // a delegates Component.onCompleted will just called the first time the
...
void tst_QQuickListView::reuse_checkThatItemsAreReused()
{
    ...
    // apps), so we have chosen to stick with this behaviour for now.
    QScopedPointer<QQuickView> window(createView());
    ...
    // Check that the reuse pool is now populated. We expect all inital items to be pooled,
    ...
    // OK, since then we can also check that binding are updated when reused).
    items = findItems<QQuickItem>(listView, "delegate");
    for (const auto item : items) {
        QString display = item->property("displayBinding").toString();
        int modelIndex = item->property("modelIndex").toInt();
    ...
    // was inside the page that was flicked out.
    listView->setContentY(0);
    QVERIFY(QQuickTest::qWaitForItemPolished(listView));
    ...
    items = findItems<QQuickItem>(listView, "delegate");
    for (const auto item : items) {
        int reusedCount = item->property("reusedCount").toInt();
